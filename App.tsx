...
export default function App() {
  ...
  // Espelham sempre o valor mais recente de transactions/transactionAllocations.
  // handleAddTransaction lê a PARTIR DAQUI (nunca da variável de estado direto),
  // porque a variável de estado só reflete o valor da render em que a função
  // foi criada - se uma mudança remota (outro dispositivo) chegou entre uma
  // render e a próxima ação do usuário, ler o estado da closure monta o novo
  // array em cima de uma foto desatualizada e "perde" o que veio de fora.
  const transactionsRef = useRef<Transaction[]>(transactions);
  const transactionAllocationsRef = useRef<TransactionAllocation[]>(transactionAllocations);
  useEffect(() => { transactionsRef.current = transactions; }, [transactions]);
  useEffect(() => { transactionAllocationsRef.current = transactionAllocations; }, [transactionAllocations]);
  ...
  const handleAddTransaction = async (t: Transaction | Transaction[]) => {
    ...
    const newTxs = [...newEntries, ...transactionsRef.current];
    const updatedAllocations = newAllocations.length > 0
      ? [...transactionAllocationsRef.current, ...newAllocations]
      : transactionAllocationsRef.current;

    // Atualiza o ref já aqui (síncrono), antes mesmo do React processar o
    // setState - protege contra dois lançamentos disparados no mesmo tick
    // (ex: duplo clique) montando ambos em cima do mesmo estado antigo.
    transactionsRef.current = newTxs;
    if (newAllocations.length > 0) {
      transactionAllocationsRef.current = updatedAllocations;
    }
