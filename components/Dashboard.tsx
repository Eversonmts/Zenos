    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      {selectedAccountId && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 shadow-2xl animate-in slide-in-from-bottom-10 duration-500 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white">
                  {monthAccounts.find(p => p.id === selectedAccountId)?.name}
                </h3>
                <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Histórico de Movimentações</p>
              </div>
              <button onClick={() => { setSelectedAccountId(null); setAccountSearch(''); }} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-600 hover:text-slate-900 dark:hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="mb-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl text-sm outline-none focus:ring-1 focus:ring-indigo-500"
                  value={accountSearch}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-emerald-500/5 border border-emerald-500/10 p-4 rounded-3xl">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Aportes</span>
                  </div>
                  <p className="text-xl font-black text-emerald-600 dark:text-emerald-400">
                    R$ {formatCurrency((monthAccounts.find(p => p.id === selectedAccountId) as any)?.monthIncome)}
                  </p>
                </div>
                <div className="bg-rose-500/5 border border-rose-500/10 p-4 rounded-3xl">
                  <div className="flex items-center gap-2 mb-1">
                    <ArrowDownCircle className="w-4 h-4 text-rose-500" />
                    <span className="text-[9px] font-black text-rose-600 uppercase tracking-widest">Saídas</span>
                  </div>
                  <p className="text-xl font-black text-rose-600 dark:text-rose-400">
                    R$ {formatCurrency((monthAccounts.find(p => p.id === selectedAccountId) as any)?.monthExpenses)}
                  </p>
                </div>
              </div>

              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border border-slate-200 dark:border-white/5">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Saldo da Conta</span>
                  <span className={`text-lg font-black tracking-tighter ${(monthAccounts.find(p => p.id === selectedAccountId)?.balance || 0) < 0 ? 'text-rose-500' : 'text-indigo-600 dark:text-indigo-400'}`}>
                    R$ {formatCurrency(monthAccounts.find(p => p.id === selectedAccountId)?.balance || 0)}
                  </span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {accountSearch === '' && (
                    <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-white/5">
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-slate-900 dark:text-slate-200 truncate">Aporte Mensal ({months[selectedMonth]})</p>
                        <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest">Automático</p>
                      </div>
                      <p className="text-xs font-black text-emerald-500">+ R$ {formatCurrency((monthAccounts.find(p => p.id === selectedAccountId) as any)?.monthIncome)}</p>
                    </div>
                  )}
                  ...
                  {filteredTransactions
                    ...
                    .map(t => {
                      const catName = data.categories.find(c => c.id === t.category_id)?.name || 'Outros';
                      return (
                        <div key={t.id} className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-white/5 last:border-0">
                          <div className="min-w-0">
                            <p className="text-xs font-bold text-slate-900 dark:text-slate-200 truncate">{t.description}</p>
                            <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest">{formatDisplayDate(t.date_at)} • {catName}</p>
                          </div>
                  ...
                  {filteredTransactions.filter(t => t.account_id === selectedAccountId).length === 0 && accountSearch !== '' && (
                    <p className="text-center py-4 text-[9px] font-black text-slate-600 uppercase italic tracking-widest">Nenhum resultado encontrado</p>
                  )}
                </div>
              </div>
            </div>
