...
// Faz upsert por `id` dentro do array que já está no cache local, em vez de
// substituir o array inteiro. Usado sempre que sabemos exatamente QUAIS itens
// mudaram (onlyNewOrUpdated) - assim um save otimista baseado num snapshot de
// estado desatualizado nunca apaga do cache local algo que outro dispositivo
// já tinha gravado ali (ex: cache local sobrescrito por uma foto antiga em
// outra aba/dispositivo, fazendo o item "sumir" até o próximo refresh).
const mergeLocalArray = <T extends { id: string }>(userId: string, key: keyof FinancialData, items: T[]) => {
  if (!items.length) return;
  const current = getLocalData(userId);
  const existing: T[] = ((current as any)?.[key]) || [];
  const map = new Map(existing.map(i => [i.id, i]));
  items.forEach(i => map.set(i.id, i));
  saveLocalData(userId, { [key]: Array.from(map.values()) } as any);
};
...
export const db = {
  ...
  saveTransactions: async (userId: string, txs: Transaction[], onlyNewOrUpdated?: Transaction | Transaction[]) => {
    ...
    // Se sabemos exatamente quais transações mudaram (onlyNewOrUpdated), faz
    // upsert só delas em cima do que já está no cache local. Só substitui o
    // array inteiro quando o caller manda a lista já resolvida como
    // autoritativa e não sabe apontar o delta (ex: fluxo de exclusão).
    if (onlyNewOrUpdated) {
      mergeLocalArray(userId, 'transactions', toUpsert);
    } else {
  ...
  saveAllocations: async (userId: string, allocations: TransactionAllocation[], onlyNewOrUpdated?: TransactionAllocation[]) => {
    if (onlyNewOrUpdated) {
      mergeLocalArray(userId, 'transaction_allocations', onlyNewOrUpdated);
    } else {
