...
export const initializeAuth = (onUserChanged: (user: Profile | null) => void) => {
  ...
  // O próprio onAuthStateChange já dispara um evento INITIAL_SESSION com a
  // sessão atual (ou null) assim que é registrado - por isso NÃO chamamos
  // supabase.auth.getSession() aqui também. Chamar os dois em paralelo fazia
  // onUserChanged (e, portanto, loadUserData no App.tsx) disparar DUAS VEZES
  // simultâneas a cada carregamento/F5 - duas cargas concorrentes de dados
  // que podiam se pisar, uma delas terminando por último com uma foto mais
  // antiga do que a outra já tinha carregado.
...
const handleAuthStateChange = async (event: string, session: any, onUserChanged: (user: Profile | null) => void) => {
  ...
  try {
    if (session?.user) {
      ...
      // INITIAL_SESSION sem sessão = não há usuário logado no carregamento atual.
